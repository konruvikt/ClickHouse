...
class S3Helper:
    ...
    def upload_file(self, file_path: Union[Path, str], s3_path: str) -> str:
        return self._upload_file_to_s3(S3_BUILDS_BUCKET, Path(file_path), s3_path)

    def download_file(self, s3_path: str, local_file_path: Union[Path, str]) -> None:
        try:
            self.client.download_file(S3_BUILDS_BUCKET, s3_path, local_file_path)
    ...
    def download_files(
        self, s3_path: str, file_suffix: str, local_directory: Union[Path, str]
    ) -> List[str]:
        ...
        for obj in objects:
            if obj.endswith(file_suffix):
                ...
                self.download_file(obj, local_file_path)
