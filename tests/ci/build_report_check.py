...
def main():
    ...
    for build_name in builds_for_check:
        ...
        if build_result.is_missing:
            ...
            if build_name == "fuzzers":
                # FIXME: fuzzers may be built or may not depending on the label
                #   we neeed to check when it should be added into report
                #   for now - just silently omit it if json report has not been found
                #   when fixed - remove that if block
                continue
